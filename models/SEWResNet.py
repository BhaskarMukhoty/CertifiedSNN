...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn1 = LIFSpikeIN()#cext_neuron.MultiStepIFNode(detach_reset=True)
        ...
        self.stride = stride
        self.sn2 = LIFSpikeIN()#cext_neuron.MultiStepIFNode(detach_reset=True)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.downsample = downsample
        self.stride = stride
        self.sn3 = LIFSpikeIN()#cext_neuron.MultiStepIFNode(detach_reset=True)
...
class SEWResNet(nn.Module):
    ...
    def __init__(self, block, layers, encoding='const', atk_encoding='const', model_mode='normal',num_classes=100, zero_init_residual=True,
        ...
        self.fc = layer.SeqToANNContainer(self.fc)
    ...
    def set_simulation_time(self, T, mode='bptt'):
        ...
        for module in self.modules():
            if isinstance(module, (LIFSpikeIN, ExpandTemporalDim)):
                module.T = T
                if isinstance(module, LIFSpikeIN):
                    module.mode = mode
        return
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, connect_f=None):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                LIFSpikeIN()#cext_neuron.MultiStepIFNode(detach_reset=True)
            )
    ...
    def _forward_impl(self, x):
        if self.T > 0:
            if self.model_mode=='normal':
                x = self.get_encoding(self.encoding, x)
                # input = input.reshape(self.T,int(input.size(0)/self.T),input.size(1),input.size(2),input.size(3))
            elif self.model_mode=='attack':
                x = self.get_encoding(self.atk_encoding, x)
            else:
