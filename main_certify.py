...
from utils import val, certify, predict
#from models import *
import attack
import copy
import torch
...
parser.add_argument('-dev','--device',default='3',type=str,help='device')
...
parser.add_argument('-m0','--m0',default=10,type=int,help='no. of noisy samples for class prediction')
parser.add_argument('-m','--m',default=100,type=int,help='no. of noisy samples for probality')
parser.add_argument('-r','--radius',default=0,type=float,help='certified radius')
parser.add_argument('-err','--error_rate',default=1.0,type=float,help='upper bound on error prob.')
parser.add_argument("--INdatapath", type=str, default="", help="Path to the ImageNet100 dataset zip file")
...
def main():
    ...
    logger = get_logger(os.path.join(log_dir, '%s.log'%(str(args.identifier)+str(args.radius))))
    ...
    check_point = torch.load(os.path.join(model_dir, args.identifier + '.pth'), map_location=torch.device('cpu'))
...
#         logger.info(json.dumps(atk_config))
        ...
        if args.attack.lower() == 'fgsm':
            print(f'FGSM, model encoding:{model.encoding} , attack encoding: {atkmodel.atk_encoding}, eps={args.eps}, T={args.time}')
            atk = attack.FGSM(atkmodel, device, forward_function=ff, eps=args.eps / 255, T=args.time)
        elif args.attack.lower() == 'pgd':
            print(f'PGD, model encoding:{model.encoding} , attack encoding: {atkmodel.atk_encoding}, eps={args.eps}, T={args.time}')
            ...
            print(f'GN,  model encoding:{model.encoding} , attack encoding: {atkmodel.atk_encoding} , eps={args.eps}, T={args.time}')
        ...
        #acc, abs = predict(model, test_loader, device, args.time, atk, m=args.m0, error_rate=args.error_rate)
        acc, abs = certify(model, test_loader, device, args.time, atk, m0=args.m0, m=args.m, error_rate=args.error_rate, radius=args.radius)
        print(acc, abs)
        logger.info(json.dumps(atk_config)+' Certified Test acc={:.3f}'.format(acc) + ' Abstain_Rate={:.3f}'.format(abs))
